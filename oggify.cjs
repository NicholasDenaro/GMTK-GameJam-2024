...
const dirsPremade = fs.readdirSync('./assets/premade/raws', {recursive: true});
dirsPremade.forEach(directory => {
...
const dirs = fs.readdirSync('./assets/raws', { recursive: true });
dirs.forEach(directory => {
  if (directory.search(/(\.wav|.mp3|.ogg)$/) > -1) {
    ...
    console.log(`starting ${path}/${file} ...`);
    try {
      fs.mkdirSync(`./assets/outputs/${path}`);
    } catch { }
    exec(
      `ffmpeg -i "./assets/raws/${path}/${file}" -y -c:a libvorbis -b:a 48k "./assets/outputs/${path}/${file.split('.')[0]}.ogg"`,
      (err, stdout) => {
        if (err) {
          console.log('error');
          console.log(err);
